...
numTel = re.compile('((?:\+\d{1,4}\-|\+\d{1,4}\s|\+\d{1,4}|\d{1,4}\-|\d{1,4}\s|\d{1,4})?\(?\d{3}\)?(?:\-|\s)?\d{3}(?:\-|\s)?\d{4})')
corEle = re.compile('[\d\w\.]+@[\d\w\.\-]+\.\w+')
...
class index(QMainWindow):
    ...
    def extraer(self):
        ...
        try:
            num_Tel = re.compile('(\+?\d{1,4}(\-|\.|\s)?)?(\(?\d\)?(\-|\.|\s)?){9}\(?\d\)?')

            cor_Ele = re.compile('(\w|\d)(\.?(\w|\d))*@(\w|\d)(\.?(\w|\d))*(\.(\d){2,4})')

            datos = requests.get(cadena)

            numerosTelefonicos = re.findall(num_Tel, datos.text)
            ...
            correosElectronicos = re.findall(cor_Ele, datos.text)
